...
# Real-robot launcher for EffortJointController.
#
# Spawns effort controller(s) for one or both arms onto the controller_manager
...
def spawn_controllers(context):
    ...
    if arm_side == 'left':
        ...
        actions.append(_spawner('arm_left_effort_joint_controller', yaml))
    elif arm_side == 'right':
        yaml = PathJoinSubstitution([pkg, 'config', 'right_arm_controllers.yaml'])
        actions.append(_spawner('arm_right_effort_joint_controller', yaml))
    else:  # both
        ...
        actions.append(_spawner('arm_left_effort_joint_controller', yaml_left))
        actions.append(_spawner('arm_right_effort_joint_controller', yaml_right))

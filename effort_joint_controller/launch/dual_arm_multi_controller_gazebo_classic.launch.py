...
# Dual-arm multi-controller Gazebo Classic launch.
#
# Spawns for each arm (left + right):
...
# Plus one shared joint_state_broadcaster for all 14 joints.
#
# Switch arm modes at runtime (std_msgs/Int32: 0=position 1=effort 2=velocity):
#   ros2 topic pub --once /arm_left_motion_controller_interface/set_mode  std_msgs/msg/Int32 "{data: 1}"
#   ros2 topic pub --once /arm_right_motion_controller_interface/set_mode std_msgs/msg/Int32 "{data: 1}"
...
def declare_actions(launch_description: LaunchDescription, launch_args: LaunchArguments):
    ...
    controllers_yaml = PathJoinSubstitution(
        [FindPackageShare('effort_joint_controller'), 'config',
         'dual_arm_multi_controllers_gazebo.yaml']
    )

    def spawner(name, inactive=False):
        args = [name, '--param-file', controllers_yaml]
        if inactive:
    ...
    # Shared broadcaster for all 14 joints.
    launch_description.add_action(spawner('joint_state_broadcaster'))

    # ── Left arm ──
    launch_description.add_action(spawner('arm_left_position_joint_controller'))
    launch_description.add_action(spawner('arm_left_velocity_joint_controller', inactive=True))
    launch_description.add_action(spawner('arm_left_effort_joint_controller', inactive=True))
    launch_description.add_action(spawner('arm_left_motion_controller_interface'))

    # ── Right arm ──
    launch_description.add_action(spawner('arm_right_position_joint_controller'))
    launch_description.add_action(spawner('arm_right_velocity_joint_controller', inactive=True))
    launch_description.add_action(spawner('arm_right_effort_joint_controller', inactive=True))
    launch_description.add_action(spawner('arm_right_motion_controller_interface'))

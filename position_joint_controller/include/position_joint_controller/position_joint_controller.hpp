...
/**
 * Position controller that subscribes to ~/commands
 * (tiago_pro_joint_controllers_msgs/JointCommand) and forwards commanded joint
 * positions to the hardware position command interfaces.
 *
 * On activation the controller reads the current joint positions from the
 * state interfaces and initialises the command buffer with those values so
 * the robot holds its current pose until the first command arrives.
 *
 * Safety features:
 *   - Per-joint position limit validation (configurable, TiagoPro defaults).
 ...
 *   publish_rate      (double, Hz, >0,     default 100.0) – max rate at which
 *                     commands are actually written to the hardware interfaces.
 *                     The controller_manager may call update() faster; extra
 *                     cycles are skipped (TriggerRate pattern).
 *
 * Topic subscribed:
 *   ~/commands  (tiago_pro_joint_controllers_msgs/msg/JointCommand)
 */
class PositionJointController : public controller_interface::ControllerInterface {
 public:
 ...
 private:
  ...
  int64_t write_period_ns_{10000000LL};  // 1e9 / publish_rate, default 100 Hz
  int64_t accumulated_period_ns_{0};

  // Filtered position state maintained across update() calls.

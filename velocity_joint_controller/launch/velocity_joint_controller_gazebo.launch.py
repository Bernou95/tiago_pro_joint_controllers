...
# Launch file for the VelocityJointController in Gazebo simulation.
# Python is required here because xacro processing needs OpaqueFunction.
#
# The velocity command interface is always available in Gazebo regardless of
# the gazebo_effort xacro flag.
#
# Usage:
...
def generate_launch_description():
    ...
    controllers_yaml = PathJoinSubstitution(
        [FindPackageShare('velocity_joint_controller'), 'config', 'controllers_gazebo.yaml']
    )
    ...
    spawn_velocity_joint_controller = Node(
        package='controller_manager',
        executable='spawner',
        arguments=[
    ...
    return LaunchDescription([
        ...
        RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[spawn_velocity_joint_controller],
            )
        ),
    ])
